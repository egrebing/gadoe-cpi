...
#See: https://tbradley1013.github.io/2018/10/01/calculating-quantiles-for-groups-with-dplyr-summarize-and-purrr-partial/
p <- c(.1, .25, .5, .75, .9)

p_names <- map_chr(p, ~paste0("STAFF_FTE_PER_1K_", .x*100, "th_pct"))
...
cpi <- read.xlsx("serve-cpi-data-output-2017-2021.xlsx") %>%
  mutate(ST_LEAID = paste0("GA-", SYSTEM_ID))

#Load the CCD LEA Universe Data for RC6
ccd.rc6 <- read.csv("../../../NCES CCD/CCD LEA Universe 2019-20/RC6 CCD 2019-20 LEAs.csv",
                    stringsAsFactors = F)

#Filter the CCD data to only GA LEA's
ccd.lea.ga <- ccd.rc6 %>%
  filter(MSTATE == "GA") #%>%
  # filter(grepl("Regular", LEA_TYPE_TEXT))

#EMG 9.9 - Filter the data to only include LEAs in the "Regular" Category
cpi <- cpi %>%
  filter(ST_LEAID %in% ccd.lea.ga$ST_LEAID)

#Get just locale codes and coordinates
...
job.codes.single <- read.csv("../CPI Files for PowerBI/GA Job Codes Denominators Single.csv", stringsAsFactors = F) %>%
...
job.codes.clustered <- read.csv("../CPI Files for PowerBI/GA Job Codes Lookup Clustered.csv", stringsAsFactors = F) %>%
  mutate(JOB_CODE = as.character(JOB_CODE),
         JOB_CODE = ifelse(nchar(JOB_CODE) == 2, paste0("0", JOB_CODE), JOB_CODE)) %>%
  mutate(Title = str_trim(Title),
...
fte.all <- bind_rows(fte.2017, fte.2018, fte.2019, fte.2020, fte.2021) %>%
  mutate(ST_LEAID = paste0("GA-", SYSTEM_ID)) %>%
...
str(fte.all)
rm(fte.2017, fte.2018, fte.2019, fte.2020, fte.2021)
...
write.csv(fte.all, "Student FTE Data 2017-2021.csv", row.names = F, na = "")
write.csv(ccd.lea.ga, "GA 2019-20 CCD LEAs.csv", row.names = F, na = "")
...
#Write to file
write.csv(cpi.fte.joined, paste0("LEA-Level CPI Data ", format(Sys.Date(), "%b %d %Y"), ".csv"),
          row.names = F, na = "")
...
#Write to File
write.csv(cpi.clusters.out, paste0("LEA-Level CPI Data Clustered Groups ", format(Sys.time(), "%b %d %Y"), ".csv"), row.names = F, na = "")
...
write.csv(state.cluster, "State-Level CPI Data 2017-2021.csv", row.names = F)
...
write.csv(resa.cluster, "RESA-Level CPI Data 2017-2021.csv", row.names = F)
...
cpi.cluster.state.summary.rows <- cpi.cluster.fact.out %>%
  filter(FISCAL_YEAR == 2021 & ST_LEAID == "GA-601") %>%
...
cpi.cluster.short.names <- read.csv("CPI Cluster Short Names.csv", stringsAsFactors = F) %>%
...
#Create Output Files with All Years FY17-FY21
write.csv(location.dim.update, "CPI Location Dimension Table.csv", row.names = F, na = "")
write.csv(cpi.fact, "CPI Fact Table.csv", row.names = F, na = "")
write.csv(cpi.cluster.fact.out, "CPI Clustered Fact Table.csv", row.names = F, na = "")
write.csv(cpi.cluster.dim.out, "CPI Cluster Dimension Table.csv", row.names = F, na = "")
write.csv(job.codes.dim, "CPI Job Code Dimension Table.csv", row.names = F, na = "")

## Create a separate set of output fact tables with only FY21 data ##
cpi.fact.fy21 <- cpi.fact %>%
...
cpi.cluster.fact.fy21 <- cpi.cluster.fact.out %>%
  filter(FISCAL_YEAR == "2021") %>%
...
cpi.cluster.fact.fy21 %>%
  count(RESA_Index, Cluster_Group, RESA_RATIO_FORMAT)

write.csv(cpi.fact.fy21, "CPI Fact Table FY21.csv", row.names = F, na = "")
write.csv(cpi.cluster.fact.fy21, "CPI Clustered Fact Table FY21.csv", row.names = F, na = "")
...
write.csv(cpi.change, "CPI Change FY17 to FY21.csv", row.names = F, na = "")
...
write.csv(resa.summary.clusters, paste0("CPI Data Summarized by Job Cluster and RESA ", format(Sys.time(), "%b %d %Y"), ".csv"),
          row.names = F, na = "")
